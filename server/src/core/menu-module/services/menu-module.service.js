  ...
  try {
    var userData = await User.findById(user.sub)
      .populate({
        ...
        select: 'isAdmin permissions'
      });
    const paths = [];
    let matchCondition = { name: name };
    ...
    const adminRole = userData.roles.find(role => {
      return role.isAdmin;
    });
    if (!adminRole) {
      matchCondition['menus.path'] = { $in: paths };
    }
...
exports.getModules = async function (user) {
  try {
    var userData = await User.findById(user.sub).populate({
      path: 'roles',
      select: 'isAdmin'
    });
    const roleAdmin = userData.roles.find(role => role.isAdmin);
    let query = { roles: { $in: userData.roles } };
    if (roleAdmin != null) {
      query = {};
    }
    var menuModule = await MenuModule
      .find(query).select('name label');
    return menuModule;
  } catch (e) {
    throw Error(e.message);
  }
